...
class Portfolio_Index(object): #Class that contains all methods for Portfolio construction
     ...
     def data_import_yf(self,ticker_list,start,end):
        ...
        for index, ticker in enumerate(ticker_list):
            ...
            data_dataframe[ticker]=data['Close']
     ...
     def create_ret_series(self,ticker_list,price_series):
         ...
         for index, ticker in enumerate(ticker_list):
            ...
            #data_dataframe[Ret_ticker_ret] = weights[index]*data_dataframe[ticker_ret]




         #data_dataframe['Ret_port_ret'] = data_dataframe['Ret_GOOG'] + \
         #                                          data_dataframe['Ret_ADSK'] + \
         #                                          data_dataframe['Ret_TSLA'] + \
         #                                          data_dataframe['Ret_V'] + \
         #                                          data_dataframe['Ret_CRM'] + \
         #                                          data_dataframe['Ret_GLD'] + \
         #                                          data_dataframe['Ret_FB'] + \
         #                                          data_dataframe['Ret_LMT'] + \
         #                                          data_dataframe['Ret_HON'] + \
         #                                          data_dataframe['Ret_BABA'] + \
         #                                          data_dataframe['Ret_HDB'] + \
         #                                          data_dataframe['Ret_AMZN'] + \
         #                                          data_dataframe['Ret_MSFT'] + \
         #                                          data_dataframe['Ret_NVDA'] + \
         #                                          data_dataframe['Ret_ARKW']

         return_series.fillna(0, inplace=True)
         #self.excel_export(data_dataframe)
         return return_series

     def create_index_list(self,ret_,list_ref):
         ...
         if not list_ref:
            new_index = 100
             ...
             new_index = list_ref[-1]  #To set the last value of in_sample index as first value for Hold-Out sample
         index_list = []
         ret_['Ret_port_ret'] = ret_['Ret_MCD'] + \
                                                   ret_['Ret_RSG'] + \
                                                   ret_['Ret_SPGI'] + \
                                                   ret_['Ret_TAL'] + \
                                                   ret_['Ret_FISV'] + \
         ...
         for idx in range(len(ret_.index) - 1):
            ret = 1 + ret_.iloc[idx+1][17]
            new_index = ret*new_index
            index_list.append(new_index)

         return index_list   #returns a list of model portfolio index values



     def to_excel(self,data_dataframe,filename):
         #Method to create an excel file from dataframe and save in the proj working directory- Not used
     ...
     def heatmap_2D(self,matrix_2D):
         #Method to create 2D heatmap
